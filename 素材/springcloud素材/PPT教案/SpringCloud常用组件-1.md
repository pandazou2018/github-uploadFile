...
随着互联网的发展，网站应用的规模不断扩大。需求的激增，带来的是技术上的压力。系统架构也因此不断的演进、升级、迭代。从单一应用，到垂直拆分，到分布式服务，到SOA，以及现在火热的微服务架构，还有在Google带领下来势汹涌的Service Mesh。我们到底是该乘坐微服务的船只驶向远方，还是偏安一隅得过且过其实生活不止眼前的苟且，还有诗和远方。所以我们今天就回顾历史，看一看系统架构演变的历程；把握现在，学习现在最火的技术架构；展望未来，争取成为一名优秀的Java工程师。



## 1.1.集中式架构

当网站流量很小时，只需一个应用，将所有功能都部署在一起，以减少部署节点和成本。此时，用于简化增删改查工作量的数据访问框架(ORM)是影响项目开发的关键。    一台tomcat并发量支持400~600用户量
...
如果你们公司全部采用Java技术栈，那么使用Dubbo作为微服务架构是一个不错的选择相反，如果公司的技术栈多样化，而且你更青睐Spring家族，那么SpringCloud搭建微服务是不二之选。在我们的项目中，我们会选择SpringCloud套件，因此我们会使用Http方式来实现服务间调用。
...
![1574169259123](assets\1574169259123.png)

![1574169509124](assets\1574169509124.png)

![1574169555069](assets\1574169555069.png)
...
- Spring Cloud config: 统一配置，把有yml文件中的配置信息，上传到git服务器中，可以在git服务器中直接改配置
- Spring Cloud Bug   消息总线， 在git服务器中改了配置文件后，不需要重其服务器，发送rabbitMQ请求后直接生效。
...
接下来，我们就一一学习SpringCloud中的重要组件。SpringBoot只要是用2.0以上的版本，JDK必须是1.8以上的版本，包含1.8.



# 4.微服务场景模拟

首先，我们需要模拟一个服务调用的场景，搭建两个工程：baoman_service_provider（服务提供方）和baoman_service_consumer（服务调用方）。方便后面学习微服务架构
...
我们新建一个项目：baoman_service_provider，创建pom工程，对外提供根据id查询用户的服务。

### 4.1.1.创建工程

![1586138790318](assets/1586138790318.png)
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <groupId>com.baoman.service</groupId>
    <artifactId>baoman_service_provider</artifactId>
...
### 4.1.2.编写代码
 ...
 ![1586139301822](assets/1586139301822.png)
...
搭建baoman_service_consumer服务消费方工程。

### 4.2.1.创建工程

与上面类似，这里不再赘述，需要注意的是，我们调用baoman_service_provider的解耦获取数据，因此不需要mybatis相关依赖了。

![1586139496393](assets/1586139496393.png)
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <groupId>com.baoman.consumer</groupId>
    <artifactId>baoman_service_consumer</artifactId>
    <version>1.0-SNAPSHOT</version>
    <parent>
        <groupId>org.springframework.boot</groupId>
        <artifactId>spring-boot-starter-parent</artifactId>
...
### 4.2.2.编写代码
 ...
 ![1586141638372](assets/1586141638372.png)
...
- baoman_service_provider：一个提供根据id查询用户的微服务。
- baoman_service_consumer：一个服务调用者，通过RestTemplate远程调用baoman_service_provider。
...
## 5.1.认识Eureka  [jʊ(ə)'riːkə]

首先我们来解决第一问题，服务的管理。

> 问题分析

在刚才的案例中，baoman_service_provider对外提供服务，需要对外暴露自己的地址。而consumer（调用者）需要记录服务提供者的地址。将来地址出现变更，还需要及时更新。这在服务较少的时候并不觉得有什么，但是在现在日益复杂的互联网环境，一个项目肯定会拆分出十几，甚至数十个微服务。此时如果还人为管理地址，不仅开发困难，将来测试、发布上线都会非常麻烦，这与DevOps的思想是背道而驰的。
...
![1586143337704](assets/1586143337704.png)
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <artifactId>baoman_eurekasd</artifactId>
    <version>1.0-SNAPSHOT</version>
    <parent>
        <groupId>org.springframework.boot</groupId>
        <artifactId>spring-boot-starter-parent</artifactId>
...
public class baomanEurekaApplication {

    public static void main(String[] args) {
        SpringApplication.run(baomanEurekaApplication.class, args);
    }
}
```
...
参照baoman_eureka，先添加SpringCloud依赖：
...
mybatis:
  type-aliases-package: com.baoman.service.pojo
eureka:
  client:
...
public class baomanServiceProviderApplication {

    public static void main(String[] args) {
        SpringApplication.run(baomanServiceApplication.class, args);
    }
}
```
...
![1574204109474](assets\1574204109474.png)
...
接下来我们修改baoman_service_consumer，尝试从EurekaServer获取服务。
...
public class baomanServiceConsumerApplication {
    ...
    public static void main(String[] args) {
        SpringApplication.run(baomanServiceConsumerApplication.class, args);
    }
}
```
...
- 服务注册中心
  ...
  Eureka的服务端应用，提供服务注册和发现功能，就是刚刚我们建立的baoman-eureka。

- 服务提供者

  提供服务的应用，可以是SpringBoot应用，也可以是其它任意技术实现，只要对外提供的是Rest风格服务即可。本例中就是我们实现的baoman_service_provider。

- 服务消费者

  消费应用从注册中心获取服务列表，从而得知每个服务方的信息，知道去哪里调用服务方。本例中就是我们实现的baoman_service_consumer。
...
我们假设要运行两个EurekaServer的集群，端口分别为：10086和10087。只需要把baoman-eureka启动两次即可。
...
2）启动第二个eurekaServer，再次修改baoman-eureka的配置：
...
Map<service-provider,Map<[USER-20190929AP:service-provider:8081](http://user-20190929ap:8081/actuator/info),实例>>
...
但是这给我们的开发带来了麻烦， 因此开发阶段我们都会关闭自我保护模式：（baoman-eureka）
...
在刚才的案例中，我们启动了一个baoman_service_provider，然后通过DiscoveryClient来获取服务实例信息，然后获取ip和端口来访问。

但是实际环境中，我们往往会开启很多个baoman_service_provider的集群。此时我们获取的服务列表中就会有多个，到底该访问哪一个呢？
...
首先参照baoman-eureka启动两个baomanProviderApplication实例，一个8081，一个8082。

 ![1574260214486](assets\1574260214486.png)
...
修改baoman_service_consumer的引导类，在RestTemplate的配置方法上添加`@LoadBalanced`注解：
...
![1574288861702](assets\1574288861702.png)
...
SpringBoot也帮我们提供了修改负载均衡规则的配置入口，在baoman_service_consumer的application.yml中添加如下配置：
...
![1574289399785](assets\1574289399785.png)
